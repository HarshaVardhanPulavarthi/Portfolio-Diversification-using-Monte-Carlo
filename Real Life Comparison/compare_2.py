...
# 3. Monte Carlo simulation for 2024 using LOG returns + NORMAL distribution
meanM = np.full(shape=(T, len(weights)), fill_value=meanreturns)
meanM = meanM.T
portfolio_sims = np.full(shape=(T, mc_sims), fill_value=0.0)

for m in range(0, mc_sims):
    Z = np.random.normal(size=(T, len(weights)))  # Gaussian noise
...
plt.xlabel('Days')
plt.title('Monte Carlo Simulated 2024 vs Actual 2024 Portfolio (Log Returns + Normal)')
plt.legend()
plt.grid(True)
plt.show()
