...
def get_data(stocks, start, end):
    ...
    returns = stockdata.pct_change().dropna()   # <-- Simple returns
...
# 3. Monte Carlo simulation using simple returns + t-distribution
...
for m in range(mc_sims):
    ...
    Z = np.random.standard_t(df, size=(T, len(weights)))   # t-distribution noise
...
# 4. Actual 2024 returns (simple returns)
...
plt.xlabel('Days')
plt.title('Monte Carlo Simulated 2024 vs Actual 2024 (Simple Returns + t-Distribution)')
plt.legend()
plt.grid(True)
